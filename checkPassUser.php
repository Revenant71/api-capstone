...
switch ($method) {
    case 'POST':
        ...
        $query_get_oldpass = "SELECT `password` FROM users";
        if ($found_id && is_numeric($found_id)) {
            ...
            if ($data && password_verify($user->currentPassword, $data['password'])){
                $hash_newPassword = password_hash($user->newPassword, PASSWORD_BCRYPT); 
                $query_set_newpass = "UPDATE users SET `password`=:pass, `updated_at`=:updated WHERE id=:id";
                // $user->oldPassword;
